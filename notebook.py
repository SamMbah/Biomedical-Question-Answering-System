...
2) Generate test-question embeddings for:
     - INSTR     (Instructor, SBERT)
     - JINA      (Jina, SBERT)
     - E5        (E5, SBERT)
     - SPECTER   (SPECTER, SBERT)
     - MPNET     (MPNET, SBERT)
     - MINILM6   (MiniLM6, SBERT)
     - MINILM12  (MiniLM12, SBERT)
     - LABSE     (LaBSE, SBERT)
     - WORD2VEC  (gensim Word2Vec on test questions, using Spacy/NLTK lemmatization & stopword removal)
     - FASTTEXT  (gensim FastText on test questions, same tokenization)
...
# We'll keep context embeddings in a dict for later
context_embeddings = {}

# 2d) Ingest each SBERT context pool into Qdrant under its model name
for fn in SBERT_PKLS:
    model_name = fn.replace("emb_","").replace(".pkl","").upper()  # yields: INSTR, JINA, E5, SPECTER, MPNET, MINILM6, MINILM12, LABSE
...
# ─── 3) LOAD TEST DATA & PREPARE TOKENIZATION ─────────────────────────────────
print("→ Step 3: Loading test questions and tokenizing…")
...
print(f"   • Test-set size: {len(questions)}")

# Tokenize all test questions for Word2Vec/FT
tokenized_q = [ tokenize(q) for q in questions ]

# ─── 4) INSTANTIATE & RUN QUESTION EMBEDDERS ─────────────────────────────────
print("\n→ Step 4: Instantiating SBERT models (GPU) and generating test embeddings…")
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
print("   • Using device:", DEVICE)

# 4a) INSTR (SBERT Instructor)
...
# 4g) LABSE (SBERT LaBSE)
labse_model = SentenceTransformer("sentence-transformers/LaBSE", device=DEVICE)

# 4h) Train “toy” Word2Vec & FastText on tokenized test questions
...
save_test_embedding("instr", instr_q_test, context_embeddings["INSTR"])
...
# (ix) WORD2VEC Q_test using *tokenized* test questions
...
# (x) FASTTEXT Q_test using *tokenized* test questions
