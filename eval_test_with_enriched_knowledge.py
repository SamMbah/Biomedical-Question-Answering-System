...
always including BioPortal or PubMed external knowledge, and writes a small
knowledge graph directly into Neo4j. Uses Qdrant for nearest-neighbor retrieval
of contexts. Implements retry logic for both OpenAI calls and Qdrant queries
to handle intermittent timeouts.
...
Environment Variables (export before running):
    ...
    export NEO4J_PWD="Neo4jPass2025!"
...
NEO4J_PWD  = os.getenv("NEO4J_PWD", "neo4j")
driver = GraphDatabase.driver(NEO4J_URI, auth=(NEO4J_USER, NEO4J_PWD))

# Before creating any nodes/edges, clear the entire DB:
...
for exp_idx, (exp_name, parts) in enumerate(experiments.items(), start=1):
    ...
    for i, row in df_sample.iterrows():
        ...
        # ─── 1) MERGE Question node with full text ────────────────────────────────
        with driver.session() as session:
            session.run(
                """
        ...
        # Collect “context” as lists of concept‐IDs
        chosen_concept_id_lists = []
        for j in best_idxs:
            cval = all_contexts[j]
            if isinstance(cval, list):
                chosen_concept_id_lists.append(cval)
            else:
                chosen_concept_id_lists.append([cval])

        # ─── 2) Enrich with external knowledge ────────────────────────────────────
        ...
        # b) Create ENRICHED_BY edge with a `source` property
        with driver.session() as session:
            session.run(
                """
                MATCH (q:Question {id: $qid}), (s:Source {name: $source})
                MERGE (q)-[r:ENRICHED_BY]->(s)
                SET r.source = $source
                """,
                qid=q_node, source=source
            )

        # ─── 3) Link to each retrieved Concept node ───────────────────────────────
        for concept_list in chosen_concept_id_lists:
            for concept_id in concept_list:
                # i) MERGE the Concept node
                with driver.session() as session:
                    session.run(
                        """
                ...
                # ii) Create USES_CONTEXT relationship
                with driver.session() as session:
                    session.run(
                        """
                        MATCH (q:Question {id: $qid}), (c:Concept {id: $cid})
                        MERGE (q)-[:USES_CONTEXT]->(c)
                        """,
                        qid=q_node, cid=concept_id
                    )

        # ─── 4) Build GPT‐3.5 prompt exactly as before ──────────────────────────
        prompt_lines = []
        prompt_lines.append("Use the following context + external knowledge to answer succinctly.\n\n")
        prompt_lines.append("Context (concept IDs):\n")
        for clist in chosen_concept_id_lists:
            prompt_lines.append("\n".join(clist) + "\n\n")
...
# Close Neo4j driver
driver.close()
print("→ All nodes & relationships have been written into Neo4j.")